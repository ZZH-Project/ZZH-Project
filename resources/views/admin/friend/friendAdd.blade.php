...
@section('title','美丽联合-添加友情链接')
@section('title-first','友情链接管理')
@section('title-second','添加友情链接')
@section('style')
...
@endsection
@section('main')
    <form id="af" action="{{url('admin/friend/check')}}" method="post">
        {{csrf_field()}}
        ...
        <input id="qt" class="myinput-main" type="text" name="fname" placeholder="请输入站点名称">
        <div id="qd" style="text-align: center;font-size: 16px;">站点域名</div>
        <input id="qt" class="myinput-main" type="text" name="fside" placeholder="请输入站点域名">
        @if(!empty($errors->all()))
