...
@section('main')
    ...
    <div style="background: white;padding: 0 10px 25px 10px;">
        <h3 style="padding-top: 20px;color: #3399ff;">网站导航列表</h3>
        <div style="padding:10px 0;">
            <div class="find">导航名称搜索：</div><input id="uf" class="myinput-main" type="text" value="">
            <input class="token" type="hidden" name="_token" value="{{csrf_token()}}">
            <a class="add" href="{{url('admin/navCate/add')}}"><i class="fa fa-plus" title="添加分类"></i></a>
            <div class="clear"></div>
        </div>
        <table class="tb">
            <tr>
            ...
            @foreach($data as $v)
                <tr class="trd">
                    ...
                    <td>{{$v->routes}}</td>
                    <td>
                        <a class="active" href="{{url('admin/navCate/edit').'/'.$v->id}}">
