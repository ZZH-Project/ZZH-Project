...
@endsection
@section('main')
    <form id="af" action="{{url('admin/wechatCate/add')}}" method="post">
        <h1 id="fh" style="text-align: center;padding-bottom: 5px;margin: 0 0 15px 0;color: #3399ff;border-bottom: 1px solid #e5e5e5">
            <span>添加微圈分类</span>
            <a style="display: none;color:#ff890a;" href='{{url('admin/wechatCate/show')}}' title='专题分类列表'>微圈分类列表</a>
        </h1>
        {{csrf_field()}}
...
@section('script')
    ...
    <script>
        ...
        $("#pt").blur(function () {
            $.ajax({
                url:"{{url('admin/wechatCate/nameCheck')}}",
                type:"get",
                data:$("#pt").serialize(),
                dataType:"json",
