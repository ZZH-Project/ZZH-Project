...
@endsection
@section('main')
    <form id="af" action="{{url('admin/wechatCate/edit')}}/{{$data->id}}" method="post">
        <h1 id="fh" style="text-align: center;padding-bottom: 5px;margin: 0 0 15px 0;color: #3399ff;border-bottom: 1px solid #e5e5e5">
            <span>修改微圈分类</span>
            <a style="display: none;color:#ff890a;" href='{{url('admin/wechatCate/show')}}' title='专题分类列表'>专题分类列表</a>
        </h1>
...
@section('script')
    ...
    <script>
        ...
        $("#pt").blur(function () {
            $.ajax({
                url:"{{url('admin/wechatCate/nameCheck')}}/"+id,
                type:"get",
                data:$("#pt").serialize(),
                dataType:"json",
        ...
        $("#et").blur(function () {
            $.ajax({
                url:"{{url('admin/wechatCate/check')}}",
                type:"get",
                data:$("#et").serialize(),
                dataType:"json",
                success:function(data) {},
